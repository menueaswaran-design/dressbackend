...
export default function NewCouponPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      await api.post("/coupons", { code, type, value: Number(value), minPurchase: Number(minPurchase), maxDiscount: maxDiscount ? Number(maxDiscount) : undefined, usageLimit: usageLimit ? Number(usageLimit) : undefined, expiresAt: new Date(expiresAt).toISOString(), isActive });
      router.push("/coupons");
    } catch (err: any) {
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-zinc-500">Create a discount coupon</p>
        </div>
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <Input type="number" value={value} onChange={(e) => setValue(e.target.value)} placeholder={type === "percentage" ? "20" : "500"} required />
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <Input type="number" value={maxDiscount} onChange={(e) => setMaxDiscount(e.target.value)} placeholder="Unlimited if empty" />
            </div>
            <div className="space-y-2">
              <Label>Usage Limit</Label>
              <Input type="number" value={usageLimit} onChange={(e) => setUsageLimit(e.target.value)} placeholder="Unlimited if empty" />
            </div>
            <div className="space-y-2">
